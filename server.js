...
app.post("/data/task", function (req, res) { // adds new task to database
	...
	db.query("SELECT MAX(sortorder) AS maxOrder FROM gantt_tasks")
		.then(function (result) { /*!*/ // assign max sort order to new task
			...
			return db.query("INSERT INTO gantt_tasks(text, start_date, duration, progress, parent,kind_task,ToDo,folder,url,mail,memo,color,textColor,owner_id,sortorder) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
			[task.text, task.start_date, task.duration, task.progress, task.parent,task.kind_task,task.ToDo,task.folder,task.url,task.mail,task.memo,task.color,task.textColor,task.owner_id, orderIndex]);
		})
...
app.put("/data/task/:id", function (req, res) {
	...
	Promise.all([
		db.query("UPDATE gantt_tasks SET text = ?, start_date = ?, duration = ?, progress = ?, parent = ?, kind_task = ?, ToDo = ?, folder = ?, url = ?, mail = ?, memo = ?, color = ?, textColor = ?, owner_id = ?  WHERE id = ?",
			[task.text, task.start_date, task.duration, task.progress, task.parent,task.kind_task,task.ToDo,task.folder,task.url,task.mail,task.memo,task.color,task.textColor,task.owner_id, sid]),
